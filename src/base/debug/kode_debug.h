...
//----------------------------------------------------------------------
  ...
  char _kode_print_buffer[256]  = {0};
  char _kode_prefix_buffer[256] = {0};

  // /home/skei/test -> test
  // returns everything after the last /

  const char* _kode_strip_path(const char* buffer) {
    const char *slash;
  ...
  void _kode_print_prefix(const char* file, const char* func, int line) {
    char buffer[256];
    const char* filename  = _kode_strip_path(file);
    strcat(_kode_prefix_buffer,"[");
    ...
    strcat(_kode_prefix_buffer,buffer);
    strcat(_kode_prefix_buffer,"] ");
  }
  ...
  void _kode_print(const char* format, ...) {
    ...
    vsprintf(_kode_print_buffer,format,args);
    strcat(_kode_prefix_buffer,_kode_print_buffer);
    //#if defined KODE_DEBUG_PRINT_SOCKET
    //  _kode_debug_socket_print(_kode_debug_prefix_buffer);
    //#else
      printf("%s",_kode_prefix_buffer);
    //#endif
    _kode_prefix_buffer[0] = 0;
    va_end(args);
  }
