#ifndef kode_integer_math_included
#define kode_integer_math_included
//----------------------------------------------------------------------
...
int KODE_AverageI(const unsigned int n, const int* ar) {
  int total = 0;
  unsigned int i=0;
  while (i<n) {
    total += ar[i];   i++;
...
uint32_t KODE_BitReverse(uint32_t v) {
  uint32_t r = v;
  int s = sizeof(v) * KODE_CHAR_BIT - 1;
  for (v >>= 1; v; v >>= 1) {
    r <<= 1;
    r |= v & 1;
    s--;
...
int KODE_ClampI(const int input, const int limit) {
  const int _t = (input > -limit) ? input : -limit;
  //return (_t > limit) ? _t : limit;
  return (_t < limit) ? _t : limit;
}

int KODE_ClampI(const int input, const int minval, const int maxval) {
...
bool KODE_IsPowerOfTwo(uint32_t x) {
  return ((x&(x-1))==0);
}
...
int KODE_MaxI(const int a, const int b) {
  return (a > b) ? a : b;
}
...
int KODE_MinI(const int a, const int b) {
  return (a < b) ? a : b;
}
...
uint32_t KODE_NextPowerOfTwo(uint32_t x) {
  x--;
...
uint32_t KODE_NumBits(uint32_t num) {
  uint32_t count = 0;
  while (num) {
    num &= (num-1);
    ++count;
...
int KODE_RmsI(const unsigned int n, const int* ar) {
  int numr = 0;
  unsigned int i=0;
  while (i<n) {
    numr += ar[i]*ar[i];
